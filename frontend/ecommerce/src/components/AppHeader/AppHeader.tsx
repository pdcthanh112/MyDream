...
export default function AppHeader() {
  ...
  console.log('HEEEEEEEEEEEEEEee', currentUser);
    ...
    <>
      <header>
        <div className="flex items-center bg-slate-400 p-1 flex-grow py-2">
          <Image src={AppLogo} alt="App Logo" style={{ width: '5rem', height: 'auto' }} className="cursor-pointer" onClick={() => router.push('/')} />
            ...
            {currentUser ? (
              <>
                <div className="cursor-pointer hover:underline">
                  {/* <p>Hello, {currentUser.userData.name}</p> */}
                  <p className="font-extrabold md:text-sm">Account & Infor</p>
                </div>
                <Card className={`hidden ${style.showCard}`}>asfasfjl;ajdfkasjf</Card>
              </>
            ) : (
              <></>
            )}
